...
| `recovery/RecoveryPage` | `GET /api/recovery/today`, `POST /read/done`, `/read/mark`, `/read/catch-up`, `GET /book/chapters/{no}`, `PUT /book/place`, `PUT /settings`, `POST /sessions`, `GET/POST/DELETE /journal`, `GET/POST/PATCH/DELETE /people`, `POST /people/{id}/contacts`, `GET /library`, `POST /import/{slot}`; `WS /api/speech/ws` via `lib/speech.ts` | the number, the day's reflection, the passage, a timer of any length with a bell, the book's two pages for today with the carry-over, the table of contents and a reader for any chapter (`BookReader`: mark read to here), the journal, the people to call (`PeoplePanel`). Four views on a phone (today · read · journal · people) and three columns on a desktop from the same markup — `pointer: coarse` decides. "Your books" (`LibraryPanel`) is where the PDFs are imported, all at once, dry run first. See [recovery.md](recovery.md) |
