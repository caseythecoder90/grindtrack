...
chat (#64) and pictures, video and stickers (#65) — and proving each piece on two phones. Do it
...
- The cluster repo's PR (the four `MEDIA_S3_*` variables on the deployment and a 110 MB ingress
  body) merged, and **applied** — CI deploys the image, not the manifests:
...
- The Hetzner bucket and its S3 key pair to hand (Cloud Console → the project → Object Storage).
...
`{"configured":true,"maxBytes":104857600}`. `configured: false` means a variable did not reach
the pod: the secret key names above, the manifest not applied (step 0), or the restart not done.
...
| Upload fails with a sentence about the bucket | `could not store that: …` — a 403 is the key pair or the bucket name; a signature error is the region (`MEDIA_S3_REGION`, default `nbg1`, must be the bucket's location) |
